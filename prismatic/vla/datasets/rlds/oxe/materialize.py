...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] not in [
        ...
        raise ValueError(
            f"Cannot load `{dataset_name}`; only EEF_POS, EEF_R6, JOINT_POS, and JOINT_POS_BIMANUAL actions supported!"
        )
    ...
    if dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POS:
        ...
        # For single-arm joint-space actions: [7 joints (absolute), gripper (absolute)]
